...
bool PowLogHE::onlyWholeFrame() {
    return false;
}

void PowLogHE::setParams(std::vector<char> p) {
    memcpy(&params, p.data(), sizeof (Params));
}

std::vector<char> PowLogHE::getParams() {
    return std::vector<char> {reinterpret_cast<char *>(&params), reinterpret_cast<char *>(&params + 1)};
}

PowLogHE::~PowLogHE() {}

std::string PowLogHE::name() {
    return "Pow-Log HE";
}

void PowLogHE::TransformHistogram(Mat<uint32_t>& hist) {

    uint32_t * pHist = hist.first() + minClipped;
    uint32_t * pEnd  = hist.first() + maxClipped + 1;

    while (pHist < pEnd) {
        *pHist = std::pow(log2(*pHist + params.additive), params.exponent);
        pHist++;
    }
}

void PowLogHE::ClipHistogram(Mat<uint32_t> &hist) {
    ...
    while (pHist < pEnd) {
        uint32_t value = *pHist > params.threshold ? params.threshold : *pHist;
        *pHist = value;
        totalClipped += value;
        ++pHist;
    }
...
void PowLogHE::MakeMap(Mat<uint8_t>& map, Mat<uint32_t>& hist) {
    ...
    memset(map.first() + maxClipped, 255, map.total() - avg);
